...
void LightsaberCrystalComponentImplementation::notifyLoadFromDatabase() {
	...
	if (color == 31 && (minimumDamage != maximumDamage || itemLevel == 0)) {
		...
		generateCrystalStats(modifier, lootQuality);
	}

	TangibleObjectImplementation::notifyLoadFromDatabase();
}

void LightsaberCrystalComponentImplementation::generateCrystalStats(float modifier, int lootQuality) {
	...
	if (color == 31) {
		int minStat = crystalData->getMinDamage() * modifier;
		int maxStat = crystalData->getMaxDamage() * modifier;
	...
	if (object->hasSkill("force_title_jedi_rank_01") || player->isPrivileged()) {
		...
		if (getColor() != 31) {
			...
			if (ownerID != 0 || player->isPrivileged()) {
				...
				alm->insertAttribute("Modifier", Math::getPrecision(modifier, 4));
...
void LightsaberCrystalComponentImplementation::fillObjectMenuResponse(ObjectMenuResponse* menuResponse, CreatureObject* player) {
	...
	if (ghost != nullptr && ghost->isPrivileged()) {
		if (getColor() == 31)
			menuResponse->addRadialMenuItem(130, 3, "Recalculate Stats");
		if (ownerID != 0)
			menuResponse->addRadialMenuItem(131, 3, "Untune Crystal");
	}
...
int LightsaberCrystalComponentImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	if (ghost != nullptr && ghost->isPrivileged()){
		if (selectedID == 130 && getColor() == 31) {
			...
			std::string tuneName = getCustomObjectName().toString();
			std::string toRemove = " \\#00FF00(tuned)\\#.";
			size_t pos = tuneName.find(toRemove);

			if (pos != std::string::npos) {
				tuneName.erase(pos, toRemove.length());
			}

			setCustomObjectName(UnicodeString(tuneName), true);
		}
	}
...
void LightsaberCrystalComponentImplementation::tuneCrystal(CreatureObject* player) {
	...
	if (ownerID == 0) {
		...
		if (getCustomObjectName().toString() == "") {
			tuneName = StringIdManager::instance()->getStringId(objectName.getFullPath().hashCode()).toString() + " \\#00FF00(tuned)\\#.";
		} else {
			tuneName = getCustomObjectName().toString() + " \\#00FF00(tuned)\\#.";
		}
...
void LightsaberCrystalComponentImplementation::updateCraftingValues(CraftingValues* values, bool firstUpdate) {
	...
	if (values->hasExperimentalAttribute("level")) {
		int level = values->getCurrentValue("level");
		setItemLevel(level);
	}
