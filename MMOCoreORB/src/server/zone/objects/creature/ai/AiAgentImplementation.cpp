...
void AiAgentImplementation::activateHAMRegeneration(int latency) {
	...
	if (isPet() && !isInCombat())
		modifier *= mountVars.mountPetOutOfCombatHAMRegenMultiplier;
...
bool AiAgentImplementation::isAttackableBy(CreatureObject* creature) {
	...
	if (isPet()) {
		...
		if ((getCreatureBitmask() & ObjectFlag::NOAIAGGRO) && isMount() && (getOptionsBitmask() & OptionBitmask::INVULNERABLE) && mountVars.mountPetAllMountsUsedByAnyone == true) {
			return false;
		}
	...
	if (creatureIsAgent) {
		...
		if (isPet() && (getCreatureBitmask() & ObjectFlag::NOAIAGGRO) && isMount() && (getOptionsBitmask() & OptionBitmask::INVULNERABLE) && mountVars.mountPetAllMountsUsedByAnyone == true) {
			return false;
		}
